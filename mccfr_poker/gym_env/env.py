...
from tools import abstraction_cards
...
class HoldemTable(Env):
    ...
    def __init__(self, num_of_players=6, initial_stacks=100, small_blind=1, big_blind=2, render=False, funds_plot=True,
        ...
        self.obs = None  # ((),)*(self.num_of_players + 1)
        self.obs_ = None
    ...
    def is_terminal(self):
        if np.any(~np.asarray(self.player_status)):
            return True
        elif self.done:
            return True
        # elif self.end_hand and self.stage.name == Stage.RIVER.name:
          #   return True
        else:
            return False
    ...
    def is_chance(self):
        if self.stage is None:
            return True
        elif self.stage.name == Stage.CHANCE.name:
            return True
        elif self.previous_stage.name == Stage.CHANCE.name:
            return True

    def reset(self):
        """Reset after game over."""
        self.observation = None
        self.last_reward = (0,) * len(self.players)
        self.info = None
        self.done = False
        ...
        if self._agent_is_autoplay() and not self.done:
            ...
            while self._agent_is_autoplay() and not self.done:
                ...
                if Action(action) not in self.legal_moves:
                    ...
                    self._execute_step(Action(action))  # Execute l'action
    ...
    def step(self, action):  # pylint: disable=arguments-differ
        ...
        if self._agent_is_autoplay():
            ...
            #log.info(f"Previous action reward for seat {self.acting_agent}: {self.last_reward}")
        return self.array_everything, self.last_reward, self.done, self.info

    def update_state(self, action):
        seq_obs = list(self.obs)

        self._update_state(action)
        new_obs = self.obs_

        for p in range(self.num_of_players + 1):
            # Own-action observation
            append_obs = ()
            if p == self.player_cycle.idx and self.distributed:
                append_obs += (action,)
    ...
    def _update_state(self, action):
        ...
        # Card distribution
        if self.stage == Stage.PREFLOP and not self.distributed:
            action += (None,)
            self.obs_ = action  # list(3) > ((C1, C2), (C3, C4), None)

        elif self.stage == Stage.PREFLOP:
            self.obs_ = (action,)*(self.num_of_players + 1)  # list(3) > ((action,) *3)
    ...
    def _illegal_move(self, action):
        ...
        self.last_reward = self.illegal_move_reward
    ...
    def _get_environment(self):
        ...
        if not self.done and not isinstance(self.current_player, bool):
        ...
        if not self.current_player:  # game over
            self.current_player = self.players[self.player_cycle.idx]
        ...
        self.player_data.equity_to_river_alive = 1 # self.current_player.equity_alive
    ...
    def _calculate_reward(self, last_action=None):
        """
        Preliminiary implementation of reward function

        - Currently missing potential additional winnings for future contributions
        """
        if last_action == Action.FOLD:
            self.last_reward = -(
                    self.community_pot + self.current_round_pot)
        else:
            self.last_reward = self.player_data.equity_to_river_alive * (self.community_pot + self.current_round_pot) - \
                               (1 - self.player_data.equity_to_river_alive) * self.player_pots[self.winner_ix]

        payoff_ = np.repeat(-self.last_reward, self.num_of_players)
        payoff_[self.winner_ix] *= -1
        ...
        _ = last_action
        if self.done:
            self.reward = self.funds_history.iloc[-1, self.player_cycle.idx] - self.funds_history.iloc[
                -2, self.player_cycle.idx]

            print(f"Reward for {self.player_cycle.idx} is {self.reward}")
            self.done = False
        else:
            pass
        """
    ...
    def _start_new_hand(self):
        ...
        #log.info("")
        #log.info("++++++++++++++++++")
        #log.info(f"Starting new hand.")
        #log.info("++++++++++++++++++")
        self.table_cards = []
        self._create_card_deck()  # Créer le deck
        self.stage = Stage.CHANCE  # Définie le premier stage
        ...
        # IF Chance NODE
        self._distribute_cards()  # Distribue les cartes à chaque joueurs
        # Recursive function

        self._initiate_round()  # Initialise le round avec les big blinds / small blinds
    ...
    def _initiate_round(self):
        ...
        # Conditions
        if self.stage.name == Stage.PREFLOP.name:
            self.min_call = 0
        elif self.stage.name == Stage.FLOP.name:
            self.min_call = self.big_blind
        elif self.stage.name == Stage.TURN.name or self.stage.name == Stage.RIVER.name:
            self.min_call = 2 * self.big_blind
    ...
    def _end_round(self):
        ...
        if self.stage == Stage.PREFLOP:
            ...
            self.previous_stage = Stage.PREFLOP
            self._distribute_cards_to_table(3)

        elif self.stage == Stage.FLOP:
            self.stage = Stage.TURN
            self.previous_stage = Stage.TURN
            self._distribute_cards_to_table(1)

        elif self.stage == Stage.TURN:
            self.stage = Stage.RIVER
            self.previous_stage = Stage.RIVER
            self._distribute_cards_to_table(1)

        elif self.stage == Stage.RIVER:
            self.previous_stage = Stage.SHOWDOWN
    ...
    def _get_winner(self):
        ...
        if sum(potential_winners) == 1:
            ...
            remaining_player_winner_ix, winning_card_type, best_hand = get_winner([player.cards
                                                                        for ix, player in enumerate(self.players) if
                                                                        potential_winners[ix]],
                                                                       self.table_cards)
            winner_ix = potential_winner_idx[remaining_player_winner_ix]
        #log.info(f"Player {winner_ix} won: {winning_card_type} with {best_hand}")
        return winner_ix
    ...
    def _next_player(self):
        ...
        if len(self.callers) > 0:
            if len(np.unique(self.player_pots)) == 1 and self.players[self.callers[-1]].last_action_in_stage == Action.CALL.name:
        ...
        if not self.current_player:
            if sum(self.player_cycle.alive) < 2:
                ...
                self.previous_stage = Stage.CHANCE
                if self.stage.name == Stage.RIVER.name:
                    self.stage = Stage.SHOWDOWN
                """
                self._end_round()
    ...
    def _distribute_cards(self):
        ...
        cards_distributed = tuple()
        for player in self.players:
            cards_distributed_ = tuple()
            player.cards = []
            if player.stack <= 0:
                continue
            for _ in range(2):
                card = np.random.randint(0, len(self.deck))
                cards_distributed_ += (self.deck.pop(card),)
                player.cards.append(cards_distributed_[-1])
            # Isomorph cards
            cards = (abstraction_cards.CARDS[player.cards[0][0] + player.cards[0][1].lower()],
                     abstraction_cards.CARDS[player.cards[1][0] + player.cards[1][1].lower()])
            player.cards[0], player.cards[1] = abstraction_cards.preflop_get_canonical(cards)[0].name, abstraction_cards.preflop_get_canonical(cards)[1].name

            cards_distributed += (cards_distributed_,)
            #log.info(f"Player {player.seat} got {player.cards} and ${player.stack}")
        self.stage = Stage.PREFLOP
        self.previous_stage = Stage.PREFLOP

        # Update obs and state
        self.update_state(cards_distributed)

    def _distribute_cards_to_table(self, amount_of_cards):
        card_distributed = tuple()
        for _ in range(amount_of_cards):
            card = np.random.randint(0, len(self.deck))
            card_distributed += (self.deck.pop(card),)
            self.table_cards.append(card_distributed[-1])
        #log.info(f"Cards on table: {self.table_cards}")
        self.distributed = False
        self.update_state(card_distributed)
