...
class Clustering(Cmatrix):
    ...
    def get_savename(self):
        savename = {"river": "river_cluster.csv",
    ...
    def skmeans(self):
        # print('clusters by sklearn:')
        cluster = KMeans(n_clusters=self.k, verbose=0)
        cluster.fit(self.points_list)
        centroids = cluster.cluster_centers_.flatten()
...
def cluster_data_main(street='river', file_path='data/', initial_method='kmean++', ifsave=True, cluster_k=5):
    params = vars(get_params())
    params['street'] = street
    params['file_path'] = file_path
    params['initial_method'] = initial_method
    params['ifsave'] = ifsave
    params['cluster_k'] = cluster_k

    #print('-- Clustering data for {}'.format(street))
    t0 = time.time()
    c = Clustering(street=params['street'], file_path=params['file_path'], k=params['k'], initialMethod=params['initialMethod'], ifsave=params['ifsave'])
    _ = c.main()
...
if __name__ == '__main__':
    params = vars(get_params())
    params['street'] = 'flop'
    params['file_path'] = 'Truedata/'
    clustering = Clustering(street=params['street'], file_path=params['file_path'], k=params['k'], initialMethod=params['initialMethod'], ifsave=params['ifsave'])
    _ = clustering.main()
