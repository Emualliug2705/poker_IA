...
from pokertools import CARDS, SUITS, RANKS
...
def flop_get_canonical(flop):
    ...
    if card1[1] == card2[1] == card3[1]:
        return (
            CARDS[card1[0] + A].name,
            CARDS[card2[0] + A].name,
            CARDS[card3[0] + A].name,
        )

    elif card1[1] == card2[1] != card3[1]:
        return (
            CARDS[card1[0] + A].name,
            CARDS[card2[0] + A].name,
            CARDS[card3[0] + B].name,
        )

    elif card1[1] == card3[1] != card2[1]:
        # Special case: if the 2nd and 3rd cards are a pair e.g. the flop is
        # [Jc, Qd, Qc].name, then our suit changes have resulted in an
        # unsorted flop! The correct canonical form is [Jc, Qc, Qd].
        return tuple(sorted([
            CARDS[card1[0] + A].name,
            CARDS[card2[0] + B].name,
            CARDS[card3[0] + A].name,
        ]))

    elif card1[1] != card2[1] == card3[1]:
        # Special case: if the 1st and 2nd cards are a pair e.g. flop is
        # [2c, 2d, 8d].name, that is isomorphic with those cards being switched
        ...
        return (
            CARDS[card1[0] + A].name,
            CARDS[card2[0] + B].name,
            CARDS[card3[0] + C].name,
        )


def preflop_get_canonical(preflop):
    card1, card2 = sorted(preflop)
    A, B = "cd"
...
def get_all_canonicals(CARDS, r=3):
    """
    Returns the set of all canonical flops. Each flop is a tuple of three
    pokertools.Card objects.
    """
    all_possible_flops = combinations(CARDS, r=r)
    if r == 3:
        return set(tuple(flop_get_canonical(flop)) for flop in all_possible_flops)
    elif r == 2:
        return set(tuple(preflop_get_canonical(preflop)) for preflop in all_possible_flops)

#------------------------------------------------------------------------------
# Suit-Isomorphs

def preflop_get_suit_isomorphs(preflop):
    card1, card2 = preflop
...
def get_translation_dict(flop):
    ...
    canonical_flop = flop_get_canonical(flop)
