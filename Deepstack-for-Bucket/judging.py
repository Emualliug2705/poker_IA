...
def judging(handnum1, handnum2, community):
    if not isinstance(community, list):
        if ' ' not in community:
            community = [community[i:i+2] for i in range(0, len(community), 2)]
        if ' ' not in handnum1:
            handnum1 = [handnum1[i:i+2] for i in range(0, len(handnum1), 2)]
        if ' ' not in handnum2:
            handnum2 = [handnum2[i:i+2] for i in range(0, len(handnum2), 2)]

    board = [Card.new(c) for c in community]
    player1_hand = [Card.new(handnum1[0]), Card.new(handnum1[1])]
    player2_hand = [Card.new(handnum2[0]), Card.new(handnum2[1])]
    hands = [player1_hand, player2_hand]
    winner = EV.get_winner(board=board, hands=hands, verbose=False)

    """
    table = ['-'.join([handnum1 + ' ' + community]),  '-'.join([handnum2 + ' ' + community]), 'D r a w']
    community = community.replace(' ', '')
    handnum1 = handnum1.replace(' ', '')
    handnum2 = handnum2.replace(' ', '')
    ...
    winner = compare_hands(best_hand1, best_hand2)
    """
