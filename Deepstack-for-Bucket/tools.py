...
if __name__ == "__main__":
    ...
    # Get ISOMORPHIC PREFLOP
    r = 2
    # all_possible_flops = combinations(CARDS.values(), r=r)
    # iso_hands = set(tuple(preflop_get_canonical(flop)) for flop in all_possible_flops)  # Get all 169 possible iso morph hands
    hand = preflop_get_canonical(deal(make_deck(), r))

    # Get ISOMORPHIC FLOP
    r = 3
    # all_possible_flops = combinations(CARDS.values(), r=r)
    # iso_hands = set(tuple(preflop_get_canonical(flop)) for flop in all_possible_flops)  # Get all 169 possible iso morph hands
    flop = get_canonical(deal(make_deck(), r))
...
r=2
#for cards in generate_permutations(r):
 #   print(cards)
